...
# D1 — first gate for the direct NIM Worker runtime.
#
# This replaces N3 and N4 in one pass, because owning the loop means the
# repository read and the validated handoff happen on the same proven mechanism:
# function calling. It exists after Codex+NIM was ruled out for the terminal
# handoff -- see docs/decisions/0001-codex-nim-worker-runtime.md.
#
# PASS requires all of:
#   - the runtime exits 0 and emits parseable JSONL
#   - a real repository read happened through the read_file tool
#   - the handoff arrived as validated tool-call arguments, accepted exactly once
#   - the persisted handoff satisfies the shipped schema exactly, re-validated
#     here rather than trusting the runtime under test
#   - the handoff's evidence contains the line only a real read could produce
#   - the sensitive filter ran over the handoff strings
#   - the working tree is untouched and no credential leaked
#
# The task prompt never names the handoff fields, so exact conformance cannot be
# explained by prompt-following.
...
# Independent re-validation. The runtime validates at its boundary, but a gate
# must not take the component under test at its word.
function Test-JsonAgainstSchema {
    ...
    if ($schemaType -eq 'string') {
        ...
        # @($null).Count is 1 in PowerShell, so a property with no enum would
        # otherwise look like an enum whose allowed set contains only $null and
        # reject every real value. Drop nulls before deciding an enum exists.
...
if ([string]::IsNullOrWhiteSpace($key)) {
    Write-Output 'NIM_WORKER_HANDOFF'
...
# Artifacts live outside the repository so the working-tree invariant stays real.
...
$handoffPath = Join-Path $runtimeRoot 'delegent-handoff.json'
...
try {
    ...
    $task = 'Read only README.md from this repository and report what it says. One piece of your evidence must be the first non-empty line after the top-level heading, copied verbatim. You are inspecting the file and must not modify anything, so report no modified files and no tests.'
...
$turnCompletedEvents = @($events | Where-Object { $_.type -eq 'turn.completed' })
$turnCompleted = $turnCompletedEvents.Count -gt 0
...
$readFileCalls = @($toolCalls | Where-Object { [string]$_.item.name -ceq 'read_file' -and $_.item.ok })
$readFileSucceeded = $readFileCalls.Count -ge 1
...
Write-Output 'boundary_reason=codex-plus-nim-could-not-provide-a-validated-handoff'
Write-Output 'schema_source=skills/delegating-work/schemas/delegent-handoff.schema.json'
Write-Output 'tool_surface=read_file,delegent_handoff'
...
Write-Output "read_file_succeeded=$([bool]$readFileSucceeded)"
