...
Assert-True ($repoRead -match 'invalid_tool_args_seen=' -and $repoRead -match 'exec_command_failed_seen=' -and $repoRead -match 'windows_process_failure_seen=') 'N3 must distinguish model tool-argument errors from Windows command-runner failures.'
Assert-True ($repoRead -match 'tool_argument_error' -and $repoRead -match 'windows_sandbox_process_error') 'N3 failure classifier must preserve both compatibility fault classes.'
