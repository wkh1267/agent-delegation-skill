...
Assert-True ($setup -match "nim-worker\.config\.toml") 'Setup must write the Codex Profile V2 nim-worker layer.'
Assert-True ($setup -match '\$profileText[\s\S]*model_provider = "nim"') 'Profile V2 layer must select the NIM provider.'
Assert-True ($setup -match '\$profileText[\s\S]*model = "\$safeModel"') 'Profile V2 layer must select the requested Nemotron model.'
Assert-True ($setup -notmatch '\[profiles\.nim-worker\]') 'Setup must not use the legacy inline profiles table with Codex 0.151.'
Assert-True ($setup -match 'profile_format=v2-layer') 'Setup output must identify the Profile V2 layout.'
